...
# Imagen principal centrada (tamaño moderado)
st.markdown("<div style='text-align: center;'>", unsafe_allow_html=True)
st.image("Gasolina.jpg", caption="Precio de la gasolina.", width=500)
st.markdown("</div>", unsafe_allow_html=True)

# Sección de Datos (centrado y tamaño normal)
st.markdown("<h2 style='text-align: center; color: #117A65;'>Datos</h2>", unsafe_allow_html=True)
st.markdown("<div style='text-align: center;'>", unsafe_allow_html=True)
st.image("DatosEntidad.png", caption="Estados.", width=400)  # <-- tamaño moderado
st.markdown("</div>", unsafe_allow_html=True)
...
# Mostrar las entradas centradas (tabla pequeña)
col_left, col_center, col_right = st.columns([1, 2, 1])
with col_center:
    st.markdown("**Entrada seleccionada:**")
    st.table(df)

# Cargar datos y entrenar el modelo (igual que antes)
...
# Calcular predicción (como float para evitar problemas con Series)
prediccion = float(b0 + b1[0]*df.at[0, 'Año'] + b1[1]*df.at[0, 'Mes'] + b1[2]*df.at[0, 'Entidad'])

# Mostrar resultado centrado y estético
